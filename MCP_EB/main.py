...
def _auto_configure_vscode():
    """自动将 Jerry EB MCP Server 写入 VS Code mcp.json"""
    server_entry = {
    ...
    try:
        ...
        config["servers"]["mcp-jerry-eb"] = server_entry
        ...
        click.echo(f'   "mcp-jerry-eb": {json.dumps(server_entry, indent=2)}')


@click.group()
def cli():
    """MCP AI Jerry EB - 智能交互反馈 MCP 服务器（Python 实现版）"""
    pass
...
def server():
    ...
    from jerry_mcp import run_server
    run_server()
...
def app(port, no_browser, no_vscode):
    ...
    click.echo(f"🚀 启动 Jerry EB Web UI → http://localhost:{port}")
    ...
    from jerry_app import run_app_server
...
def version():
    """显示版本信息"""
    click.echo("MCP AI Jerry EB v1.0.0")
    click.echo("Python 重新实现版，功能等同于 mcp-ai-jerry.exe")
    click.echo(f"Python: {sys.version}")


@cli.command()
def test():
    """测试 Jerry EB 各组件是否正常运行"""
    import httpx
    ...
    if MCP_JSON_PATH.exists():
        with open(MCP_JSON_PATH, "r", encoding="utf-8") as f:
            config = json.load(f)
        if "mcp-jerry-eb" in config.get("servers", {}):
            click.echo("✅ VS Code MCP 已配置 mcp-jerry-eb")
        else:
            click.echo("⚠️  VS Code MCP 未配置 mcp-jerry-eb，运行 'python main.py app' 自动配置")
    else:
...
def copilot_server():
    """启动 Copilot CLI Jerry MCP 服务器（同 server 命令）"""
    from jerry_mcp import run_server
    run_server()
